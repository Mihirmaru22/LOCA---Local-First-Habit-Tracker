...
// MARK: - Composed View (Output)

@Model
final class ComposedView {
    var id: UUID = UUID()
    var timestamp: Date
    var question: String  // The question being answered (e.g., "Am I happier since...")
    var startDate: Date
    var endDate: Date
    ...
    var counterfactualVariable: String?  // e.g., "what if you hadn't started the job?"
...
struct TimelinePoint: Codable {
    ...
    let renderingStyle: String  // "crisp", "soft", "speculative"
}
...
struct AnnotationPoint: Codable {
    let timestamp: Date
    let text: String
    let position: String  // "above", "below"
    let targetState: String  // "energy", "stress", "focus", "mood"
}
